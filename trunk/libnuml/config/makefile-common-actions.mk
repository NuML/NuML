...
## Author(s)   : NUML Team <numl-team@caltech.edu>
## Organization: California Institute of Technology
## Created     : 2004-06-17
## Revision    : $Id: makefile-common-actions.mk 9975 2009-08-14 19:35:24Z ajouraku $
## $HeadURL: https://numl.svn.sourceforge.net/svnroot/numl/branches/rel-4-0-0/config/makefile-common-actions.mk $
##
##<!---------------------------------------------------------------------------
...
# numbering very easy: it's a direct mapping of the libnuml version number.
...
# Although we don't explicitly use libtool in the hand-written libNUML
...
ifeq "$(HOST_TYPE)" "darwin"
  ...
  define libnumlrun
	env DYLD_LIBRARY_PATH="$(RUN_LDPATH):$(DYLD_LIBRARY_PATH):." srcdir=. $(LIBTOOL) -dlopen $(TOP_BUILDDIR)/src/libnuml.la --mode=execute $(1)
  endef

else

  define libnumlrun
	env LD_LIBRARY_PATH="$(RUN_LDPATH):$(LD_LIBRARY_PATH):." srcdir=. $(LIBTOOL) -dlopen $(TOP_BUILDDIR)/src/libnuml.la --mode=execute $(1)
  endef
...
$(TOP_SRCDIR)/src/numl/config.h: src/numl/stamp-h1
	@if test ! -f $@; then \
	  rm -f src/numl/stamp-h1; \
	  $(MAKE) src/numl/stamp-h1; \
	else :; fi

$(TOP_SRCDIR)/src/numl/stamp-h1: $(TOP_SRCDIR)/./src/numl/config.h.in \
	     $(TOP_BUILDDIR)/config.status
	@rm -f src/numl/stamp-h1
	cd $(TOP_BUILDDIR) && $(SHELL) ./config.status src/numl/config.h
...
# input.  This is therefore really only meant to be used in the libnuml
# 'src' directory.
