...
dnl Revision    : $Id: python.m4 10019 2009-08-18 10:12:24Z ajouraku $
dnl $HeadURL: https://sbrml.svn.sourceforge.net/svnroot/sbrml/branches/rel-4-0-0/config/python.m4 $
dnl
dnl <!-------------------------------------------------------------------------
...
[
    ...
    *darwin*) 
	...
	if test `expr "${PYTHON_PREFIX}" ':' '/Library/Frameworks/.*'` -ne 0; then
	  ...
	  macosx_version=`sw_vers -productVersion | cut -d"." -f1,2` 
        ...
        # MacOS X note: this MUST remain .so even though we use .dylib for libsbrml.
        PYTHON_EXT="so"
	;;
    *cygwin* | *mingw*) 
  ...
  LIBSBRML_OPTIONS="$LIBSBRML_OPTIONS USE_PYTHON"
